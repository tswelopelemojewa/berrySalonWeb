...
// }, [date, serviceDurationMinutes]);
  ...
  const slots = useMemo(() => {
    ...
    const startLimitMinutes = workingHoursStart * 60;
    const endLimitMinutes = workingHoursEnd * 60;

    const slotCount = Math.ceil(minutesInDay / serviceDurationMinutes);
    for (let i = 0; i < slotCount; i++) {
      const minutesOffset = startMinutes + i * serviceDurationMinutes;
      if (minutesOffset < startLimitMinutes || minutesOffset + serviceDurationMinutes > endLimitMinutes) {
        continue; // skip slots outside working hours
      }
          ...
          <Form.Group as={Row} className="mb-2" controlId="formService">
            ...
            <Col sm="9">
              ...
              >
                ...
                {services
                  .filter(service => {
                    // Hide personal blockage services for normal customers
                    if (!isAdmin) {
                      ...
                      {Math.floor(service.duration_minutes / 60) > 0
                        ? `${Math.floor(service.duration_minutes / 60)} hour${
                            ...
                            service.duration_minutes % 60 > 0
                              ? `${service.duration_minutes % 60} minute${
                                  service.duration_minutes % 60 > 1 ? 's' : ''
                                }`
                              : ''
                          }`
                        : `${service.duration_minutes} minute${
                            service.duration_minutes > 1 ? 's' : ''
                          }`}
                    </option>
                  ))}
              </Form.Select>
